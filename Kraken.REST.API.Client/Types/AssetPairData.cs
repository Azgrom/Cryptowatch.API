...
/// <summary>
/// Represents a response for asset pair data retrieved from the Kraken API.
/// </summary>
/// <remarks>
/// The AssetPairResponse encapsulates a collection of asset pair data where the key is the asset pair identifier,
/// and the value is the detailed information of the asset pair represented by <c>AssetPairData</c>.
/// This class is primarily used as part of the API client for deserializing responses related to asset pair information.
/// </remarks>
public sealed record AssetPairResponse
{
    public AssetPairResponse(Dictionary<string, AssetPairData> pairs) { Pairs = pairs; }
    public Dictionary<string, AssetPairData> Pairs { get; init; }
}

/// <summary>
/// Represents detailed information about a specific asset pair used in the Kraken API.
/// </summary>
/// <remarks>
/// The AssetPairData record encapsulates metadata and attributes regarding a trading pair,
/// including identifiers, trading lot specifications, leverage information, associated fees,
/// margin policies, and trading limits. It serves as a core data model for handling responses
/// related to asset pairs in Kraken’s API integration.
/// </remarks>
public record AssetPairData
{
...
public class AssetPairJsonConverter : JsonConverter<Result<AssetPairResponse>>
{
    ...
    public override Result<AssetPairResponse> Read(
    ...
    )
    {
        ...
        if (jsonReader.TokenType != JsonTokenType.StartObject)
            return new Error(ErrorCodes.UnexpectedTokenErrorCode,
                $"Expected StartObject but found {jsonReader.TokenType}");

        // Read the "error" property.
        var nextResult = jsonReader.ReadNext();
        if (nextResult.IsFailure) return new Error(ErrorCodes.ReadingTokenErrorCode, nextResult.Error);
        ...
        if (errorSweepResult.IsFailure) return new Error(ErrorCodes.SweepingPropertyErrorCode, errorSweepResult.Error);
        ...
        var pairs = new Dictionary<string, AssetPairData>();
        ...
        while (true)
        {
            ...
            if (jsonReader.TokenType == JsonTokenType.EndObject) break; // End of the "result" object.
            ...
            if (jsonReader.TokenType != JsonTokenType.StartObject)
                return new Error(ErrorCodes.UnexpectedTokenErrorCode,
                    $"Expected StartObject for asset pair '{pairKey}' but found {jsonReader.TokenType}");

            var assetPairResult = ReadAssetPair(ref jsonReader, pairKey);
            if (assetPairResult.IsFailure) return assetPairResult.Error;
    ...
    public override void Write(Utf8JsonWriter writer, Result<AssetPairResponse> value, JsonSerializerOptions options)
    {
        ...
        if (value.IsSuccess)
        {
            foreach (var kvp in value.Value.Pairs)
            {
                ...
                writer.WriteStartArray();
                foreach (var num in kvp.Value.LeverageBuy) writer.WriteNumberValue(num);
                writer.WriteEndArray();

                writer.WritePropertyName("leverage_sell");
                writer.WriteStartArray();
                foreach (var num in kvp.Value.LeverageSell) writer.WriteNumberValue(num);
                writer.WriteEndArray();
    ...
    /// </summary>
    private static Result<AssetPairData> ReadAssetPair(ref Utf8JsonReader jsonReader, string pairKey)
    {
        ...
        while (true)
        {
            ...
            if (jsonReader.TokenType != JsonTokenType.PropertyName)
                return new Error(ErrorCodes.UnexpectedTokenErrorCode,
                    $"Expected property name in asset pair '{pairKey}' but found {jsonReader.TokenType}");
            ...
            switch (propertyName)
            {
                case "altname":
                    if (jsonReader.TokenType != JsonTokenType.String)
                        return new Error(ErrorCodes.UnexpectedTokenErrorCode,
                            $"Expected string for 'altname' in asset pair '{pairKey}', found {jsonReader.TokenType}");
                ...
                case "wsname":
                    if (jsonReader.TokenType != JsonTokenType.String)
                        return new Error(ErrorCodes.UnexpectedTokenErrorCode,
                            $"Expected string for 'wsname' in asset pair '{pairKey}', found {jsonReader.TokenType}");
                ...
                case "aclass_base":
                    if (jsonReader.TokenType != JsonTokenType.String)
                        return new Error(ErrorCodes.UnexpectedTokenErrorCode,
                            $"Expected string for 'aclass_base' in asset pair '{pairKey}', found {jsonReader.TokenType}");
                ...
                case "base":
                    if (jsonReader.TokenType != JsonTokenType.String)
                        return new Error(ErrorCodes.UnexpectedTokenErrorCode,
                            $"Expected string for 'base' in asset pair '{pairKey}', found {jsonReader.TokenType}");
                ...
                case "aclass_quote":
                    if (jsonReader.TokenType != JsonTokenType.String)
                        return new Error(ErrorCodes.UnexpectedTokenErrorCode,
                            $"Expected string for 'aclass_quote' in asset pair '{pairKey}', found {jsonReader.TokenType}");
                ...
                case "quote":
                    if (jsonReader.TokenType != JsonTokenType.String)
                        return new Error(ErrorCodes.UnexpectedTokenErrorCode,
                            $"Expected string for 'quote' in asset pair '{pairKey}', found {jsonReader.TokenType}");
                ...
                case "lot":
                    if (jsonReader.TokenType != JsonTokenType.String)
                        return new Error(ErrorCodes.UnexpectedTokenErrorCode,
                            $"Expected string for 'lot' in asset pair '{pairKey}', found {jsonReader.TokenType}");
                ...
                case "pair_decimals":
                    if (jsonReader.TokenType != JsonTokenType.Number)
                        return new Error(ErrorCodes.UnexpectedTokenErrorCode,
                            $"Expected number for 'pair_decimals' in asset pair '{pairKey}', found {jsonReader.TokenType}");
                    if (!jsonReader.TryGetInt32(out pair_decimals))
                        return new Error(ErrorCodes.ParsingErrorCode,
                            $"Failed to parse 'pair_decimals' for asset pair '{pairKey}'");
                    found_pair_decimals = true;
                    break;
                case "cost_decimals":
                    if (jsonReader.TokenType != JsonTokenType.Number)
                        return new Error(ErrorCodes.UnexpectedTokenErrorCode,
                            $"Expected number for 'cost_decimals' in asset pair '{pairKey}', found {jsonReader.TokenType}");
                    if (!jsonReader.TryGetInt32(out cost_decimals))
                        return new Error(ErrorCodes.ParsingErrorCode,
                            $"Failed to parse 'cost_decimals' for asset pair '{pairKey}'");
                    found_cost_decimals = true;
                    break;
                case "lot_decimals":
                    if (jsonReader.TokenType != JsonTokenType.Number)
                        return new Error(ErrorCodes.UnexpectedTokenErrorCode,
                            $"Expected number for 'lot_decimals' in asset pair '{pairKey}', found {jsonReader.TokenType}");
                    if (!jsonReader.TryGetInt32(out lot_decimals))
                        return new Error(ErrorCodes.ParsingErrorCode,
                            $"Failed to parse 'lot_decimals' for asset pair '{pairKey}'");
                    found_lot_decimals = true;
                    break;
                case "lot_multiplier":
                    if (jsonReader.TokenType != JsonTokenType.Number)
                        return new Error(ErrorCodes.UnexpectedTokenErrorCode,
                            $"Expected number for 'lot_multiplier' in asset pair '{pairKey}', found {jsonReader.TokenType}");
                    if (!jsonReader.TryGetInt32(out lot_multiplier))
                        return new Error(ErrorCodes.ParsingErrorCode,
                            $"Failed to parse 'lot_multiplier' for asset pair '{pairKey}'");
                    found_lot_multiplier = true;
                    break;
                case "leverage_buy":
                    if (jsonReader.TokenType != JsonTokenType.StartArray)
                        return new Error(ErrorCodes.UnexpectedTokenErrorCode,
                            $"Expected array for 'leverage_buy' in asset pair '{pairKey}', found {jsonReader.TokenType}");
                    var levBuyList = new List<int>();
                    while (true)
                    {
                        var r = jsonReader.ReadNext();
                        if (r.IsFailure) return new Error(ErrorCodes.ReadingTokenErrorCode, r.Error);
                        if (jsonReader.TokenType == JsonTokenType.EndArray) break;
                        if (jsonReader.TokenType != JsonTokenType.Number)
                            return new Error(ErrorCodes.UnexpectedTokenErrorCode,
                                $"Expected number in 'leverage_buy' in asset pair '{pairKey}', found {jsonReader.TokenType}");
                        if (!jsonReader.TryGetInt32(out int lev))
                            return new Error(ErrorCodes.ParsingErrorCode,
                                $"Failed to parse number in 'leverage_buy' for asset pair '{pairKey}'");
                        levBuyList.Add(lev);
                    }
                ...
                case "leverage_sell":
                    if (jsonReader.TokenType != JsonTokenType.StartArray)
                        return new Error(ErrorCodes.UnexpectedTokenErrorCode,
                            $"Expected array for 'leverage_sell' in asset pair '{pairKey}', found {jsonReader.TokenType}");
                    ...
                    while (true)
                    {
                        ...
                        if (jsonReader.TokenType != JsonTokenType.Number)
                            return new Error(ErrorCodes.UnexpectedTokenErrorCode,
                                $"Expected number in 'leverage_sell' in asset pair '{pairKey}', found {jsonReader.TokenType}");
                        if (!jsonReader.TryGetInt32(out int lev))
                            return new Error(ErrorCodes.ParsingErrorCode,
                                $"Failed to parse number in 'leverage_sell' for asset pair '{pairKey}'");
                        levSellList.Add(lev);
                    }
                ...
                case "fees":
                    if (jsonReader.TokenType != JsonTokenType.StartArray)
                        return new Error(ErrorCodes.UnexpectedTokenErrorCode,
                            $"Expected array for 'fees' in asset pair '{pairKey}', found {jsonReader.TokenType}");
                    ...
                    while (true)
                    {
                        ...
                        if (jsonReader.TokenType != JsonTokenType.StartArray)
                            return new Error(ErrorCodes.UnexpectedTokenErrorCode,
                                $"Expected sub-array for 'fees' in asset pair '{pairKey}', found {jsonReader.TokenType}");
                        decimal feeVolume  = 0;
                        decimal feePercent = 0;
                        var     r1         = jsonReader.ReadNext();
                        if (r1.IsFailure) return new Error(ErrorCodes.ReadingTokenErrorCode, r1.Error);
                        if (jsonReader.TokenType != JsonTokenType.Number)
                            return new Error(ErrorCodes.UnexpectedTokenErrorCode,
                                $"Expected number for fee volume in 'fees' for asset pair '{pairKey}', found {jsonReader.TokenType}");
                        if (!jsonReader.TryGetDecimal(out feeVolume))
                            return new Error(ErrorCodes.ParsingErrorCode,
                                $"Failed to parse fee volume in 'fees' for asset pair '{pairKey}'");
                        var r2 = jsonReader.ReadNext();
                        if (r2.IsFailure) return new Error(ErrorCodes.ReadingTokenErrorCode, r2.Error);
                        if (jsonReader.TokenType != JsonTokenType.Number)
                            return new Error(ErrorCodes.UnexpectedTokenErrorCode,
                                $"Expected number for fee percent in 'fees' for asset pair '{pairKey}', found {jsonReader.TokenType}");
                        if (!jsonReader.TryGetDecimal(out feePercent))
                            return new Error(ErrorCodes.ParsingErrorCode,
                                $"Failed to parse fee percent in 'fees' for asset pair '{pairKey}'");
                        var r3 = jsonReader.ReadNext();
                        if (r3.IsFailure) return new Error(ErrorCodes.ReadingTokenErrorCode, r3.Error);
                        if (jsonReader.TokenType != JsonTokenType.EndArray)
                            return new Error(ErrorCodes.UnexpectedTokenErrorCode,
                                $"Expected end of sub-array for 'fees' in asset pair '{pairKey}', found {jsonReader.TokenType}");
                ...
                case "fees_maker":
                    if (jsonReader.TokenType != JsonTokenType.StartArray)
                        return new Error(ErrorCodes.UnexpectedTokenErrorCode,
                            $"Expected array for 'fees_maker' in asset pair '{pairKey}', found {jsonReader.TokenType}");
                    ...
                    while (true)
                    {
                        ...
                        if (jsonReader.TokenType != JsonTokenType.StartArray)
                            return new Error(ErrorCodes.UnexpectedTokenErrorCode,
                                $"Expected sub-array for 'fees_maker' in asset pair '{pairKey}', found {jsonReader.TokenType}");
                        decimal feeVolume  = 0;
                        decimal feePercent = 0;
                        var     r1         = jsonReader.ReadNext();
                        if (r1.IsFailure) return new Error(ErrorCodes.ReadingTokenErrorCode, r1.Error);
                        if (jsonReader.TokenType != JsonTokenType.Number)
                            return new Error(ErrorCodes.UnexpectedTokenErrorCode,
                                $"Expected number for fee volume in 'fees_maker' for asset pair '{pairKey}', found {jsonReader.TokenType}");
                        if (!jsonReader.TryGetDecimal(out feeVolume))
                            return new Error(ErrorCodes.ParsingErrorCode,
                                $"Failed to parse fee volume in 'fees_maker' for asset pair '{pairKey}'");
                        var r2 = jsonReader.ReadNext();
                        if (r2.IsFailure) return new Error(ErrorCodes.ReadingTokenErrorCode, r2.Error);
                        if (jsonReader.TokenType != JsonTokenType.Number)
                            return new Error(ErrorCodes.UnexpectedTokenErrorCode,
                                $"Expected number for fee percent in 'fees_maker' for asset pair '{pairKey}', found {jsonReader.TokenType}");
                        if (!jsonReader.TryGetDecimal(out feePercent))
                            return new Error(ErrorCodes.ParsingErrorCode,
                                $"Failed to parse fee percent in 'fees_maker' for asset pair '{pairKey}'");
                        var r3 = jsonReader.ReadNext();
                        if (r3.IsFailure) return new Error(ErrorCodes.ReadingTokenErrorCode, r3.Error);
                        if (jsonReader.TokenType != JsonTokenType.EndArray)
                            return new Error(ErrorCodes.UnexpectedTokenErrorCode,
                                $"Expected end of sub-array for 'fees_maker' in asset pair '{pairKey}', found {jsonReader.TokenType}");
                ...
                case "fee_volume_currency":
                    if (jsonReader.TokenType != JsonTokenType.String)
                        return new Error(ErrorCodes.UnexpectedTokenErrorCode,
                            $"Expected string for 'fee_volume_currency' in asset pair '{pairKey}', found {jsonReader.TokenType}");
                ...
                case "margin_call":
                    if (jsonReader.TokenType != JsonTokenType.Number)
                        return new Error(ErrorCodes.UnexpectedTokenErrorCode,
                            $"Expected number for 'margin_call' in asset pair '{pairKey}', found {jsonReader.TokenType}");
                    if (!jsonReader.TryGetInt32(out margin_call))
                        return new Error(ErrorCodes.ParsingErrorCode,
                            $"Failed to parse 'margin_call' for asset pair '{pairKey}'");
                    found_margin_call = true;
                    break;
                case "margin_stop":
                    if (jsonReader.TokenType != JsonTokenType.Number)
                        return new Error(ErrorCodes.UnexpectedTokenErrorCode,
                            $"Expected number for 'margin_stop' in asset pair '{pairKey}', found {jsonReader.TokenType}");
                    if (!jsonReader.TryGetInt32(out margin_stop))
                        return new Error(ErrorCodes.ParsingErrorCode,
                            $"Failed to parse 'margin_stop' for asset pair '{pairKey}'");
                    found_margin_stop = true;
                    break;
                case "ordermin":
                    if (jsonReader.TokenType != JsonTokenType.String)
                        return new Error(ErrorCodes.UnexpectedTokenErrorCode,
                            $"Expected string for 'ordermin' in asset pair '{pairKey}', found {jsonReader.TokenType}");
                ...
                case "costmin":
                    if (jsonReader.TokenType != JsonTokenType.String)
                        return new Error(ErrorCodes.UnexpectedTokenErrorCode,
                            $"Expected string for 'costmin' in asset pair '{pairKey}', found {jsonReader.TokenType}");
                ...
                case "tick_size":
                    if (jsonReader.TokenType != JsonTokenType.String)
                        return new Error(ErrorCodes.UnexpectedTokenErrorCode,
                            $"Expected string for 'tick_size' in asset pair '{pairKey}', found {jsonReader.TokenType}");
                ...
                case "status":
                    if (jsonReader.TokenType != JsonTokenType.String)
                        return new Error(ErrorCodes.UnexpectedTokenErrorCode,
                            $"Expected string for 'status' in asset pair '{pairKey}', found {jsonReader.TokenType}");
                ...
                case "long_position_limit":
                    if (jsonReader.TokenType != JsonTokenType.Number)
                        return new Error(ErrorCodes.UnexpectedTokenErrorCode,
                            $"Expected number for 'long_position_limit' in asset pair '{pairKey}', found {jsonReader.TokenType}");
                    if (!jsonReader.TryGetInt32(out long_position_limit))
                        return new Error(ErrorCodes.ParsingErrorCode,
                            $"Failed to parse 'long_position_limit' for asset pair '{pairKey}'");
                    found_long_position_limit = true;
                    break;
                case "short_position_limit":
                    if (jsonReader.TokenType != JsonTokenType.Number)
                        return new Error(ErrorCodes.UnexpectedTokenErrorCode,
                            $"Expected number for 'short_position_limit' in asset pair '{pairKey}', found {jsonReader.TokenType}");
                    if (!jsonReader.TryGetInt32(out short_position_limit))
                        return new Error(ErrorCodes.ParsingErrorCode,
                            $"Failed to parse 'short_position_limit' for asset pair '{pairKey}'");
        ...
        if (!found_altname)
            return new Error(ErrorCodes.MissingPropertyErrorCode,
                $"Missing property 'altname' for asset pair '{pairKey}'");
        if (!found_wsname)
            return new Error(ErrorCodes.MissingPropertyErrorCode,
                $"Missing property 'wsname' for asset pair '{pairKey}'");
        if (!found_aclass_base)
            return new Error(ErrorCodes.MissingPropertyErrorCode,
                $"Missing property 'aclass_base' for asset pair '{pairKey}'");
        if (!found_base)
            return new Error(ErrorCodes.MissingPropertyErrorCode,
                $"Missing property 'base' for asset pair '{pairKey}'");
        if (!found_aclass_quote)
            return new Error(ErrorCodes.MissingPropertyErrorCode,
                $"Missing property 'aclass_quote' for asset pair '{pairKey}'");
        if (!found_quote)
            return new Error(ErrorCodes.MissingPropertyErrorCode,
                $"Missing property 'quote' for asset pair '{pairKey}'");
        if (!found_lot)
            return new Error(ErrorCodes.MissingPropertyErrorCode, $"Missing property 'lot' for asset pair '{pairKey}'");
        if (!found_pair_decimals)
            return new Error(ErrorCodes.MissingPropertyErrorCode,
                $"Missing property 'pair_decimals' for asset pair '{pairKey}'");
        if (!found_cost_decimals)
            return new Error(ErrorCodes.MissingPropertyErrorCode,
                $"Missing property 'cost_decimals' for asset pair '{pairKey}'");
        if (!found_lot_decimals)
            return new Error(ErrorCodes.MissingPropertyErrorCode,
                $"Missing property 'lot_decimals' for asset pair '{pairKey}'");
        if (!found_lot_multiplier)
            return new Error(ErrorCodes.MissingPropertyErrorCode,
                $"Missing property 'lot_multiplier' for asset pair '{pairKey}'");
        if (!found_leverage_buy)
            return new Error(ErrorCodes.MissingPropertyErrorCode,
                $"Missing property 'leverage_buy' for asset pair '{pairKey}'");
        if (!found_leverage_sell)
            return new Error(ErrorCodes.MissingPropertyErrorCode,
                $"Missing property 'leverage_sell' for asset pair '{pairKey}'");
        if (!found_fees)
            return new Error(ErrorCodes.MissingPropertyErrorCode,
                $"Missing property 'fees' for asset pair '{pairKey}'");
        if (!found_fees_maker)
            return new Error(ErrorCodes.MissingPropertyErrorCode,
                $"Missing property 'fees_maker' for asset pair '{pairKey}'");
        if (!found_fee_volume_currency)
            return new Error(ErrorCodes.MissingPropertyErrorCode,
                $"Missing property 'fee_volume_currency' for asset pair '{pairKey}'");
        if (!found_margin_call)
            return new Error(ErrorCodes.MissingPropertyErrorCode,
                $"Missing property 'margin_call' for asset pair '{pairKey}'");
        if (!found_margin_stop)
            return new Error(ErrorCodes.MissingPropertyErrorCode,
                $"Missing property 'margin_stop' for asset pair '{pairKey}'");
        if (!found_ordermin)
            return new Error(ErrorCodes.MissingPropertyErrorCode,
                $"Missing property 'ordermin' for asset pair '{pairKey}'");
        if (!found_costmin)
            return new Error(ErrorCodes.MissingPropertyErrorCode,
                $"Missing property 'costmin' for asset pair '{pairKey}'");
        if (!found_tick_size)
            return new Error(ErrorCodes.MissingPropertyErrorCode,
                $"Missing property 'tick_size' for asset pair '{pairKey}'");
        if (!found_status)
            return new Error(ErrorCodes.MissingPropertyErrorCode,
                $"Missing property 'status' for asset pair '{pairKey}'");
        if (!found_long_position_limit)
            return new Error(ErrorCodes.MissingPropertyErrorCode,
                $"Missing property 'long_position_limit' for asset pair '{pairKey}'");
        if (!found_short_position_limit)
            return new Error(ErrorCodes.MissingPropertyErrorCode,
                $"Missing property 'short_position_limit' for asset pair '{pairKey}'");
        ...
        var assetPair = new AssetPairData
        {
            Altname            = altname,
            Wsname             = wsname,
